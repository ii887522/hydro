...
/**
 * It is a binary search tree that automatically balances itself when operations such as insertions and deletions are
 * applied to maintain the efficiency of all the operations supported in the tree. It allows the user to access objects
 * in the tree in any way. Objects with duplicate keys in the tree are not supported. All objects in the tree must have
 * unique keys. If the user performs an inorder traversal of the tree, then all objects retrieved from the tree are
 * sorted in ascending order by key only.
 */
export default class AvlTree<K, V> {
  ...
  /**
   * It removes multiple objects by their respective `key`. No objects will be removed from the tree if all the `key`
   * received is not found in the tree.
   *
  ...
  /**
   * It removes an existing object by `key`. No objects will be removed from the tree if the `key` received is not
   * found in the tree.
   *
  ...
  remove (key: K): void {
    ...
    const alternate = target.left !== undefined
      ? this.maxNodeFrom(target.left)
      : (target.right !== undefined ? this.minNodeFrom(target.right) : undefined)
    const startNodeToRefresh = alternate !== undefined
      ? (alternate.parent === target ? alternate : alternate.parent)
      : target.parent
  ...
  private refresh (start: Node<K, V> | undefined): void {
    ...
    while (current !== undefined) {
      current.leftHeight = current.left !== undefined
        ? Math.max(current.left.leftHeight, current.left.rightHeight) + 1
        : 0
      current.rightHeight = current.right !== undefined
        ? Math.max(current.right.leftHeight, current.right.rightHeight) + 1
        : 0
  ...
  private flipRight (from: Node<K, V>): void {
    ...
    requireDefined(requireDefined(from.parent).left).rightHeight = leftNode !== undefined
      ? Math.max(leftNode.leftHeight, leftNode.rightHeight) + 1
      : 0
    requireDefined(from.parent).leftHeight = Math.max(
      requireDefined(requireDefined(from.parent).left).leftHeight,
      requireDefined(requireDefined(from.parent).left).rightHeight
    ) + 1
    requireDefined(from.parent).rightHeight = Math.max(from.leftHeight, from.rightHeight) + 1
  ...
  private flipLeft (from: Node<K, V>): void {
    ...
    requireDefined(requireDefined(from.parent).right).leftHeight = rightNode !== undefined
      ? Math.max(rightNode.leftHeight, rightNode.rightHeight) + 1
      : 0
    requireDefined(from.parent).leftHeight = Math.max(from.leftHeight, from.rightHeight) + 1
    requireDefined(from.parent).rightHeight = Math.max(
      requireDefined(requireDefined(from.parent).right).leftHeight,
      requireDefined(requireDefined(from.parent).right).rightHeight
    ) + 1
  ...
  /**
   * It allows the user to convert the tree into an array by performing a preorder traversal of the tree and retrieves
   * each object visited and stored them in an array and returns it.
   *
  ...
  /**
   * It allows the user to convert the tree into an array by performing an inorder traversal of the tree and retrieves
   * each object visited and stored them in an array and returns it.
   *
  ...
  /**
   * It allows the user to convert the tree into an array by performing a postorder traversal of the tree and retrieves
   * each object visited and stored them in an array and returns it.
   *
   * @returns An array that contains objects from the tree.
   */
